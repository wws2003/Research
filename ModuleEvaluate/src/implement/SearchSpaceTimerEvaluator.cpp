...
//Override
void SearchSpaceTimerEvaluatorImpl::evaluate(MatrixCollectionPtr pMatrixCollection) {
	...
	for(size_t i = 0; i < numberOfCases; i++) {
			...
			findTargetAndMeasureTime(pMatrixCollection, pTarget, m_pMatrixDistanceCalculator, m_epsilon, pFindResultIter, m_pTimer, &searchTime);
